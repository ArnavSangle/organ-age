...
v4_report_tech.py
=================
Generate a dark-themed, two-page "AI-style" technical PDF report for a
single subject.

This report targets a technical / AI-research audience and uses a dark
background with cyan and orange accent colours.

Page 1 – Model-derived aging profile:
  * Dark-background summary block with chronological age, composite
    biological age, delta, z-score, organ counts, and burden metric.
  * Explanatory paragraph on the multi-modal embedding methodology.
  * Per-organ table with full dark styling.

Page 2 – Visual summary:
  * Radar chart of calibrated organ z-scores (dark theme).
  * Bar chart of per-organ age deltas (dark theme).
  * Horizontal CI error-bar plot (dark theme).

Output is written to ``reports/v4/tech/OrganAge_tech_<subject_id>.pdf``.
"""
import argparse
...
def radar_plot(ax, organs, zscores):
    """
    Draw a dark-themed polar radar chart of per-organ calibrated z-scores.
    ...
    ax.plot(angles, values, linewidth=2, color="#00d8ff")
    ax.fill(angles, values, alpha=0.25, color="#00d8ff")
    ax.set_xticks(angles[:-1])
    ax.set_xticklabels(organs, fontsize=9, color="white")
    ax.set_yticklabels([])
    ax.set_title("Organ Z-Scores (Calibrated)", fontsize=12, pad=15, color="white")


def delta_bar_plot(ax, organs, deltas):
    """
    Draw a dark-themed bar chart of per-organ age deltas.

    Negative deltas (biologically younger) are drawn in cyan; positive
    deltas (biologically older) in orange.
    ...
    """
    colors = ["#00d8ff" if d < 0 else "#ff7f0e" for d in deltas]
    ax.bar(organs, deltas, color=colors)
    ax.axhline(0, color="white", linewidth=1)
    ax.set_facecolor("#111111")
    ax.set_ylabel("Δ (Organ Age – Chronological Age)", fontsize=10, color="white")
    ax.set_xticklabels(organs, rotation=45, ha="right", fontsize=9, color="white")
    ax.tick_params(colors="white")
    ax.set_title("Organ Age Deltas", fontsize=12, color="white")


def ci_plot(ax, chrono_age, organs, cal_ages, ci_low, ci_high):
    """
    Draw a dark-themed horizontal error-bar plot of calibrated organ ages.
    ...
    ax : matplotlib.axes.Axes
        A standard Cartesian subplot.
    chrono_age : float
        Subject's chronological age, drawn as a red dashed reference line.
    organs : list[str]
    ...
    ax.axvline(chrono_age, color="#ff4d4d", linestyle="--", linewidth=2, label="Chronological Age")
    ax.set_yticks(y)
    ax.set_yticklabels(organs, fontsize=9, color="white")
    ax.set_xlabel("Age (years)", fontsize=10, color="white")
    ax.tick_params(colors="white")
    ax.set_title("Organ Ages with Confidence Intervals", fontsize=12, color="white")
    ax.legend(fontsize=8, facecolor="#111111", edgecolor="#666666", labelcolor="white")


def build_report(subject_id: str,
                 cal_path: str = CAL_PATH,
                 panel_path: str = PANEL_PATH,
                 out_dir: str = "reports/v4/tech"):
    """
    Build and save the two-page dark-themed technical PDF report.
    ...
    out_pdf = out_dir_path / f"OrganAge_tech_{subject_id}.pdf"
    ...
    with PdfPages(out_pdf) as pdf:
        ...
        title = f"OrganAge-v4 AI Report\nSubject: {subject_id}"
        fig.suptitle(title, fontsize=18, color="#00d8ff", y=0.96)
        ...
        summary_text = (
            ...
            f"Organs included: {n_org}\n"
            f"Organs with |z| ≥ 2: {n_abn}\n"
            f"Mean |organ z| (burden): {burden:.2f}"
        )

        fig.text(0.08, 0.78, "Model-Derived Aging Profile", fontsize=14,
                 weight="bold", color="#00d8ff")
        fig.text(0.08, 0.57, summary_text, fontsize=11, color="white")

        expl = (
            "This composite biological age is computed from organ-specific\n"
            "cross-modal embeddings using a reliability-weighted fusion of\n"
            "transcriptomic (GTEx) and imaging-derived (IXI, CheXpert) models.\n"
            "Negative deltas indicate organs younger than expected for age."
        )
        fig.text(0.08, 0.47, expl, fontsize=9, color="#cccccc")
        ...
        tbl_df = df_org[cols]
        table_ax = fig.add_axes([0.05, 0.05, 0.9, 0.38])
        table_ax.axis("off")
        ...
        fig.suptitle(f"OrganAge-v4 – Model Visualization for {subject_id}",
                     fontsize=16, y=0.98, color="#00d8ff")
        pdf.savefig(fig)
        plt.close(fig)

    print(f"[TECH] Saved report -> {out_pdf}")


def main():
    """
    Command-line entry point for the tech-style report generator.

    Parses subject ID, data paths, and output directory from CLI arguments
    and delegates to ``build_report``.
    """
    parser = argparse.ArgumentParser(description="Generate tech-style OrganAge-v4 report.")
    parser.add_argument("--subject", type=str, required=True)
    parser.add_argument("--calibrated", type=str, default=CAL_PATH)
    parser.add_argument("--panel", type=str, default=PANEL_PATH)
    parser.add_argument("--outdir", type=str, default="reports/v4/tech")
